 ...
 * <p/>
 * Test with
 * <pre>{@code echo "FILE CONTENTS" | curl localhost:8080/backend/upload -F "upfile=@-"}</pre>
 * or
 * <pre>{@code cat test-upload-data.txt | curl localhost:8080/backend/upload -F "upfile=@-"}</pre>
 *
 * User: hartmann
 * Date: 10/19/13
 */
public class FileUploadServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            bytesWritten = writeStreamToFile(fileStream, new File(OUT_DIR,fileName));
        } catch (IOException e) {
